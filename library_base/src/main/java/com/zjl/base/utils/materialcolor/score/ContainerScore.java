...
package com.zjl.base.utils.materialcolor.score;

import androidx.core.util.Pair;

import com.zjl.base.utils.materialcolor.hct.Cam16;
 ...
 *
 * 注意：该类为自定义，默认Score类筛选了更黑的颜色，我们放宽对它的限制
 */
public final class ContainerScore {
   ...
   * @param noUiColorDeep 如果没有取到合适的颜色，我们选取一个是否选取最深的颜色，如果不是，我们选取一个最浅的颜色
   ...
   */
  public static Pair<Boolean, List<Integer>> score(Map<Integer, Integer> colorsToPopulation, boolean noUiColorDeep) {
    ...
    Collections.sort(entryList, new Score.ScoredComparator());
    ...
    if (colorsByScoreDescending.isEmpty()) {
      // 都没找到，我们加入灰色为默认色
      if(noUiColorDeep){
        handleDeepColor = true;
        colorsByScoreDescending.add(0xFF888888); // Gray
      } else {
        handleDeepColor = true;
        colorsByScoreDescending.add(0xFFE6E1E5); // LightGray
      }
    }

    return new Pair<>(handleDeepColor, colorsByScoreDescending);
  }
