...
public class CamSolver {
   ...
   *     RGB coordinates.
   */
  static ArrayList<double[]> edgePoints(double y) {
    double kR = Y_FROM_LINRGB[0];
    double kG = Y_FROM_LINRGB[1];
    double kB = Y_FROM_LINRGB[2];
    double[][] points =
        new double[][] {
          new double[] {y / kR, 0.0, 0.0},
          new double[] {(y - 100 * kB) / kR, 0.0, 100.0},
          new double[] {(y - 100 * kG) / kR, 100.0, 0.0},
          new double[] {(y - 100 * kB - 100 * kG) / kR, 100.0, 100.0},
          new double[] {0.0, y / kG, 0.0},
          new double[] {100.0, (y - 100 * kR) / kG, 0.0},
          new double[] {0.0, (y - 100 * kB) / kG, 100.0},
          new double[] {100.0, (y - 100 * kR - 100 * kB) / kG, 100.0},
          new double[] {0.0, 0.0, y / kB},
          new double[] {100.0, 0.0, (y - 100 * kR) / kB},
          new double[] {0.0, 100.0, (y - 100 * kG) / kB},
          new double[] {100.0, 100.0, (y - 100 * kR - 100 * kG) / kB},
        };
  ...
  static double[][] bisectToSegment(double y, double targetHue) {
    ArrayList<double[]> vertices = edgePoints(y);
    double[] left = vertices.get(0);
    double[] right = left;
    double leftHue = hueOf(left);
    double rightHue = leftHue;
    boolean uncut = true;
    for (int i = 1; i < vertices.size(); i++) {
      double[] mid = vertices.get(i);
  ...
  static double inverseChromaticAdaptation(double adapted) {
    ...
    double base = max(0, 27.13 * adaptedAbs / (400.0 - adaptedAbs));
